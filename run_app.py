...
@app.route('/username/<f_name>/<l_name>')
def build_username(f_name, l_name):
    """
...
def search_user():
    ...
    user_count = 0
    for user in users:
        if search_param in user.lower():
            user_count += 1
    return 'Found {} users that match with search: \"{}\"'.format(user_count, search_param)
