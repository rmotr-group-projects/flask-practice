...
def build_username(first_name, last_name):
    ...
    """
    #f_initial = first_name.lower()[0]
    #l_name = last_name.lower()
    return first_name.lower()[0] + last_name.lower()
...
def search_user():
    ...
    #users_ = [user.lower() for user in users]
    counter = len([user for user in users if name.lower() in user.lower()])
    return 'Found {} users that match with search: "{}"'.format(counter, name)
