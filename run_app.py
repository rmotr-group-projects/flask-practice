...
def search_user():
    ...
    a_string = request.args.get('search')
    a_count =  sum([1 for user in users if a_string.lower() in user.lower()])
    return 'Found {} users that match with search: "{}"'.format(a_count,a_string)
