...
def build_username(first_name, last_name):
    ...
    """
    first_letter = first_name.lower()[0]
    subsequent_letters = last_name.lower()
    return first_letter+subsequent_letters
...
def search_user():
    ...
    number = sum([1 for user in users if (search.lower() in user.lower())])
    return 'Found {} users that match with search: "{}"'.format(number, search.lower())
